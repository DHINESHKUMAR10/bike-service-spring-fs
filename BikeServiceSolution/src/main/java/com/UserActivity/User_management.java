...
public class User_management {
	...
	public String adduser(HttpServletRequest request) throws SQLException {
		int status = 0;
		int count = 0;
		PreparedStatement st = (PreparedStatement) con.prepareStatement("insert into users_details values (?,?,?,?,?)");
		st.setString(1, request.getParameter("user_name"));
		st.setString(2, request.getParameter("user_passw"));
		st.setString(3, request.getParameter("email"));
		st.setString(4, request.getParameter("phone_number"));
		
		int maxVal = 0;
		try (Statement getMaxStmt = (Statement) con.createStatement()) {
		    ResultSet maxResult = getMaxStmt.executeQuery("SELECT MAX(id) FROM users_details");
		    if (maxResult.next()) {
		        maxVal = maxResult.getInt(1);
		    }
		}

		// Increment the value by 1
		int updatedValue = maxVal + 1;
		st.setInt(5, updatedValue);
	...
	public static List<ServiceDetails> getAllRecords() throws SQLException {
		...
		PreparedStatement ps = (PreparedStatement) con
				.prepareStatement("select * from service where visibility = 'yes'and availability = 'yes' order by id");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public static List<BookDetail> getAllRecords(String email, String pattern) throws SQLException {
		...
		PreparedStatement ps = (PreparedStatement) con.prepareStatement(
				"select * from booking where state " + pattern + " and email='" + email + "' order by booking_date");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			...
			PreparedStatement ps2 = (PreparedStatement) con
					.prepareStatement("select * from service where id=" + rs.getInt(3));
